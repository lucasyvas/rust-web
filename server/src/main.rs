...
type GraphqlSchema = RootNode<'static, graphql::checklist::Query, graphql::checklist::Mutation>;
...
async fn run_grpc() -> Result<()> {
    let mut pool = core::database::create_pool(&env::var("DATABASE_URL")?).await?;
    core::database::create_schema(Arc::make_mut(&mut pool)).await?;
...
async fn run_graphql() -> Result<()> {
    ...
    let checklist_schema = warp::any().map(move || GraphqlSchema::new(Query, Mutation));
